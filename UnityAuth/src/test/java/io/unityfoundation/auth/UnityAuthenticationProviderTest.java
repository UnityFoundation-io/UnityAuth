...
/**
 * Integration tests for UnityAuthenticationProvider.
 * Tests authentication logic including credential validation and failure scenarios.
 */
...
@MicronautTest
class UnityAuthenticationProviderTest {
    ...
    @Test
    void login_successWithValidCredentials() {
    ...
    void login_failsWithInvalidPassword() {
        UsernamePasswordCredentials creds = new UsernamePasswordCredentials("person1@test.io", "wrongpassword");
        HttpRequest<?> request = HttpRequest.POST("/api/login", creds);
    ...
    @Test
    void login_failsWithNonExistentUser() {
        UsernamePasswordCredentials creds = new UsernamePasswordCredentials("nonexistent@test.io", "test");
        HttpRequest<?> request = HttpRequest.POST("/api/login", creds);
    ...
    void login_failsWithEmptyPassword() {
        // FINDING: Empty password causes INTERNAL_SERVER_ERROR instead of UNAUTHORIZED
        // This should ideally return 401 UNAUTHORIZED for security best practices
        ...
        // Current behavior: returns 500 INTERNAL_SERVER_ERROR
        assertEquals(HttpStatus.INTERNAL_SERVER_ERROR, exception.getStatus());
    }

    @Test
    void login_failsWithEmptyUsername() {
        // FINDING: Empty username causes INTERNAL_SERVER_ERROR instead of UNAUTHORIZED
        // This should ideally return 401 UNAUTHORIZED for security best practices
    ...
    @Test
    void login_isCaseSensitiveForPassword() {
        // "test" is the correct password, "Test" should fail
        UsernamePasswordCredentials creds = new UsernamePasswordCredentials("person1@test.io", "Test");
        HttpRequest<?> request = HttpRequest.POST("/api/login", creds);
    ...
    @Test
    void login_successForDisabledUserAtAuthTime() {
        // Note: disabled@test.io can still log in; the disabled check happens at permission check time
        // This test documents the current behavior
        UsernamePasswordCredentials creds = new UsernamePasswordCredentials("disabled@test.io", "test");
        HttpRequest<?> request = HttpRequest.POST("/api/login", creds);

        HttpResponse<BearerAccessRefreshToken> response = client.toBlocking()
                .exchange(request, BearerAccessRefreshToken.class);

        // Currently disabled users CAN authenticate but are blocked at permission check
        assertEquals(HttpStatus.OK, response.getStatus());
    }
    ...
    void login_returnsValidJwtToken() {
        ...
        BearerAccessRefreshToken bearer = response.body();
        assertNotNull(bearer);
        assertNotNull(bearer.getAccessToken());
        // JWT tokens have 3 parts separated by dots
        String[] parts = bearer.getAccessToken().split("\\.");
        assertEquals(3, parts.length, "JWT should have 3 parts (header.payload.signature)");
    }

    @Test
    void login_failsWithCaseSensitiveEmail() {
        // Test that email lookup is case-sensitive (or not, depending on DB config)
        // person1@test.io exists, PERSON1@TEST.IO might not match depending on collation
        UsernamePasswordCredentials creds = new UsernamePasswordCredentials("PERSON1@TEST.IO", "test");
        HttpRequest<?> request = HttpRequest.POST("/api/login", creds);

        // This test documents the current behavior - may succeed or fail based on DB collation
        try {
            HttpResponse<BearerAccessRefreshToken> response = client.toBlocking()
                    .exchange(request, BearerAccessRefreshToken.class);
            // If we get here, email is case-insensitive
            assertEquals(HttpStatus.OK, response.getStatus());
        } catch (HttpClientResponseException e) {
            // If we get here, email is case-sensitive
